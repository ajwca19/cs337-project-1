...
def cleansing_finding_keyword(tweets, aw_word_list):
    ...
    keywords_1 = ["nominate", "nominee", "nomination", "win", "winner", "award"]
    keywords_2 = ["best"]
    keywords_3 = list(set(aw_word_list) - set(keywords_1) - set(keywords_2))

    return [txt for txt in tweets if any(keyword in txt.lower() for keyword in keywords_1) and any(
        keyword in txt.lower() for keyword in keywords_2) and any(keyword in txt.lower() for keyword in keywords_3)]
...
# Importing wikipedia and imdb modules to check if correct nominees exist 
...
# Mapping the nominees list to the award list 
def nominee_award_find_in_tweet(aw_list, aw_word_list, no_list, tweets):
